...
export default class InformeDiario extends React.Component {
	...
	getData() {
		...
		request("consumos_" + ambiente + "/consultaventasgrupo-" + ambiente, { m: 4, periodo: "D" }, (res) => {
			res.then((res) => {
					...
					<ScrollView
						...
						<View style={{
							...
							<View style={{
								...
								<TouchableOpacity style={styles.chartsSpace} onPress={() => { this.props.navigation.navigate('INFORME DIARIO DETALLE', { date: this.state.dataSelected.fechaReferencia}) }}>
									<Arc
										r={80}
